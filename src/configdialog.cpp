        ...
        commandsList(clist), themePath(tpth), themesList(thm), logPath(lpth), logging(log), serverWindow(srvw), usersList(ulist), maxAway(maxa), nickChar(nichar), colors(clrs), sameFont(sfnt)
{
    ...
    new FXButton(closeframe, _("Cancel"), NULL, this, ID_CANCEL, BUTTON_DEFAULT|LAYOUT_RIGHT|FRAME_RAISED|FRAME_THICK, 0,0,0,0, 20,20);
    ...
    new FXCheckButton(cframe, _("Use same font for user list and commandline"), &targetSameFont, FXDataTarget::ID_VALUE, CHECKBUTTON_NORMAL|LAYOUT_FILL_X|LAYOUT_SIDE_LEFT);
    ...
    new FXLabel(otherpane, _("Other settings"), NULL, LAYOUT_LEFT);
...
long ConfigDialog::OnAddCommand(FXObject*, FXSelector, void*)
{
    ...
    new FXButton(buttonframe, _("Cancel"), NULL, &dialog, FXDialogBox::ID_CANCEL, BUTTON_DEFAULT|FRAME_RAISED|FRAME_THICK|LAYOUT_CENTER_X, 0,0,0,0, 32,32,5,5);
...
{
    FXString combo;
    FXString available[8] = { "ban", "ctcp", "join", "me", "nick", "notice", "part", "quit"};
    for(FXint i=0; i<8; i++)
    {
        FXbool exist = false;
